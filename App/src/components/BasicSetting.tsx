...
const breakpointIcons: IconDefinition[] = [
  faMobile,
  faPortrait,
  faTablet,
  faDeskpro,
    ...
    <div className="flex flex-col gap-10 px-3 py-8 bg-background3 rounded-md">
      <div className="flex flex-row gap-5 justify-between">
        <div className="flex items-center gap-3 mb-2">
          ...
          <Tooltip
            ...
            defaultValue={prop.swiperDefault}
          />
        </div>
        {type === 'number' && (
          <Input
      ...
      {prop.breakpoints && (
        <div className="flex flex-row gap-3 py-10 w-full overflow-x-scroll border-border2 border-t">
          {Object.entries(prop.breakpoints as BreakpointObject).map(
            ([key, value]) => (
              <div key={key}>
                ...
                {prop.breakpoints && (
                  <Input
                    ...
                    onChange={(e) =>
                      updateWaterfall(prop.attr, e.target.value, key)
                    }
                  />
                )}
              </div>
