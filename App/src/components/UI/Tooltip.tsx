...
function Tooltip({ content, defaultValue, position = 'right' }: TooltipProps) {
  ...
  const arrowClasses = {
    right: 'left-[-6px] top-1/2 transform -translate-y-1/2 w-0 h-0 border-t-6 border-r-6 border-b-6 border-transparent border-r-gray-900',
  ...
  return (
    ...
    >
      ...
      {visible && (
        <div className={`absolute z-10 bg-gray-900 text-white text-sm py-2 px-2 rounded-sm shadow-lg whitespace-normal max-w-xs w-max transition-opacity duration-200 ease-in-out opacity-100 ${positionClasses[position]}`}>
          {/* Triangle */}
