...
export const WaterfallProvider = ({ children }: WaterfallProviderProps) => {
  ...
  const [waterfallSettings, setWaterfallSettings] = useState<
    WaterfallCategory[] | null
  >(null);
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null);
  const [elementSelected, setElementSelected] = useState<AnyElement | null>(
    null
  );
  const [waterfallSelected, setWaterfallSelected] = useState<string | null>(
    null
  );
  const [loadedWaterfall, setLoadedWaterfall] =
    useState<LoadedWaterfall | null>(null);
  ...
  async function loadWaterfall() {
    ...
    if (el?.customAttributes) {
      ...
      const updatedProps: WaterfallCategory[] = JSON.parse(
        JSON.stringify(defaultWaterfallSettings)
      );
          ...
          if (item) {
            if (breakpoint) {
              ...
              if (item.breakpoints && breakpoint in item.breakpoints) {
                item.breakpoints[breakpoint as keyof typeof item.breakpoints] =
                  attr.value;
              }
            } else {
  ...
  function updateWaterfall(
    ...
    const updatedData: WaterfallCategory[] = waterfallSettings.map(
      (category) => {
        return {
          ...category,
      ...
      (waterfallSettings ?? []).flatMap((category) =>
        [
          ...
          if (setting.breakpoints) {
            await Promise.all(
              Object.entries(setting.breakpoints).map(
